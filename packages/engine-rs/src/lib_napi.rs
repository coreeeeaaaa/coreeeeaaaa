...
pub fn append_quantum(record: Buffer, path: Option<String>) -> Result<u32> {
    ...
    let mut q = *bytemuck::from_bytes::<UemQuantum>(&record);
    let pb = path.map(PathBuf::from).unwrap_or_else(|| PathBuf::from(".core/core.uem"));
    let mut ledger = Ledger::open(pb).map_err(|e| EngineError::Io(e.to_string()))?;
...
pub fn read_all(path: Option<String>) -> Result<Vec<Buffer>> {
    let pb = path.map(PathBuf::from).unwrap_or_else(|| PathBuf::from(".core/core.uem"));
    let ledger = Ledger::open_with_genesis(pb, genesis_quantum()).map_err(|e| EngineError::Io(e.to_string()))?;
...
pub fn genesis(path: Option<String>) -> Result<()> {
    let pb = path.map(PathBuf::from).unwrap_or_else(|| PathBuf::from(".core/core.uem"));
    Ledger::open(pb).map_err(|e| EngineError::Io(e.to_string()))?;
    Ok(())
}

// Temporarily comment out NAPI objects that are causing build issues
/*
#[napi(object)]
